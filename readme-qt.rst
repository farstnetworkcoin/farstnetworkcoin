Farstcoin-qt: Qt4 GUI for Farstcoin
===============================
...
Alternatively, install Qt Creator and open the `Farstcoin-qt.pro` file.

An executable named `Farstcoin-qt` will be built.
...
A warning for people using the *static binary* version of Farstcoin on a Linux/UNIX-ish system (tl;dr: **Berkely DB databases are not forward compatible**).

The static binary version of Farstcoin is linked against libdb5.3.
...
installed causes Farstcoin-qt to crash intermittently.  The issue has been reported as `launchpad bug 857790`_, but
isn't yet fixed.
